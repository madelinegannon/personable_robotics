...
//    params.add(match_all.set("Match_All_Params", false));
    gravity.set("Gravity", ofVec3f(0,0,0), ofVec3f(-20,-20,-20), ofVec3f());
    match_all.set("Match_All_Params", false);
...
//--------------------------------------------------------------
void AgentController::set_target(int index, ofNode tgt)
{
    if (index >=0 && index < NUM_AGENTS){
        targets[index] = tgt;
    }
    else{
        ofLog() << "index out of bounds" << endl;
    }
}


//--------------------------------------------------------------
void AgentController::set_targets(vector<ofxGizmo*> tgts)
{
    for (int i=0; i<NUM_AGENTS; i++){
        ofNode tgt;
        //    tgt.setGlobalOrientation(target->getRotate());
...
//--------------------------------------------------------------
void AgentController::set_positions(vector<ofxGizmo*> tgts)
{
    for (int i=0; i<NUM_AGENTS; i++){
        ofNode tgt;
        //    tgt.setGlobalOrientation(target->getRotate());
        tgt.setGlobalPosition(tgts[i]->getTranslation());
        agents[i]->set_pose(tgt);
        targets[i] = tgt;
    }
}

//--------------------------------------------------------------
ofNode AgentController::get_pose(int id)
{
    return agents[id]->pose;
}
